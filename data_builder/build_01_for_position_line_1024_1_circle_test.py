...
# 输出路径
label_path = os.path.join(proj_path, 'datasets/train_1024_1_circle_pos/test/label/')
...
dir_idx = 0
for random in np.transpose(randoms):
    if dir_idx == 500: break
    time_start = time.time()

    if (random[0] ** 2 + random[1] ** 2) ** 0.5 < 2400:
        continue

    # Interrupt index
    dir_idx = dir_idx + interrupt
    ...
    # File params
    label_file_name = os.path.join(label_path, str(dir_idx) + ".txt")
    data_file_name = os.path.join(data_path, str(dir_idx) + ".txt")
    ...
    print('Count ' + str(dir_idx) + ' Time cost = %fs' % (time.time() - time_start))
    dir_idx += 1
